...
enum SetupPackageProfileEdit {
  ...
  static func prepare(
    source: URL, fragment: URL, needsWiring: Bool,
    removing identities: Set<HomebrewPackageIdentity>
    ...
    for kind in HomebrewPackageIdentity.Kind.allCases {
      let names = Set(identities.filter { $0.kind == kind }.map(\.name))
      if names.isEmpty { continue }
      let selector = CanonicalTOMLSelector(
        configuration: edit.after, table: "packages",
        key: kind == .formula ? "exclude_formulae" : "exclude_casks")
      guard selector.assignments.count == 1 else {
        throw SetupPackageAdoptionError(
          "Cannot locate the selected \(kind.rawValue) exclusions in \(source.path).")
      }
      let range = selector.assignments[0].contentRange
      let assignment = String(edit.after[range])
      edit.after.replaceSubrange(range, with: try remove(names, kind: kind, from: assignment))
    }
    if needsWiring {
  ...
  private static func remove(
    _ names: Set<String>, kind: HomebrewPackageIdentity.Kind, from assignment: String
  ) throws -> String {
    ...
    for (index, range) in tokens.enumerated() where assignment[range] != "," {
      ...
      if names.contains(HomebrewPackageIdentity(kind: kind, name: value).name) {
