...
local root = __MACARCHY_STATE_ROOT_LUA__
...
function M.verify_plugins()
  ...
  local expected_lockfile = vim.fn.stdpath("config") .. "/lazy-lock.json"
  if vim.fn.fnamemodify(config.options.lockfile, ":p") ~= vim.fn.fnamemodify(expected_lockfile, ":p") then
    error("Macarchy: Lazy must use " .. expected_lockfile, 0)
  end

  local lockfile = assert(io.open(expected_lockfile, "r"))
  local decoded, lock = pcall(vim.json.decode, lockfile:read("*a"))
  lockfile:close()
  if not decoded or type(lock) ~= "table" then
    error("Macarchy: invalid Lazy lock", 0)
  end

  local root_prefix = vim.fn.fnamemodify(config.options.root, ":p"):gsub("/+$", "") .. "/"
  local failures = {}
  for _, plugin in ipairs(lazy.plugins()) do
    local directory = vim.fn.fnamemodify(plugin.dir, ":p")
    if plugin.url and directory:sub(1, #root_prefix) == root_prefix then
      local pin = lock[plugin.name]
      local result = vim.system({ "git", "-C", plugin.dir, "rev-parse", "HEAD" }, { text = true }):wait()
      local commit = result.code == 0 and vim.trim(result.stdout or "") or nil
      if type(pin) ~= "table" or type(pin.commit) ~= "string" then
        table.insert(failures, plugin.name .. ": missing lock")
      elseif commit ~= pin.commit then
        table.insert(failures, plugin.name .. ": not at lock")
      end
    end
  end
  if #failures > 0 then
    table.sort(failures)
    error("Macarchy plugin verification failed: " .. table.concat(failures, ", "), 0)
  end
end
  ...
  local started, start_error = watcher:start(root, {}, function(watch_error)
    if watch_error then
      vim.schedule(function()
        vim.notify(
          "Macarchy: canonical theme watcher failed: " .. watch_error,
