...
struct RuntimeEnvironmentTests {
  ...
  func versionCommandHasStableHumanAndJSONContracts() throws {
    ...
    #expect(try runner.executeConcise() == "0.2.2")
    #expect(
      try runner.execute(json: false)
        == "Macarchy 0.2.2 (\(revision), macos-arm64, unmanaged)"
    )
    ...
    #expect(object["version"] as? String == "0.2.2")
  ...
  private var packagedBuildInformation: String {
    "{\"schema_version\":1,\"version\":\"0.2.2\",\"revision\":\"\(revision)\"}\n"
  }
