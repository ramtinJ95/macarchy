...
struct RuntimeEnvironmentTests {
  ...
  func versionCommandHasStableHumanAndJSONContracts() throws {
    ...
    #expect(try runner.executeConcise() == "0.4.0")
    #expect(
      try runner.execute(json: false)
        == "Macarchy 0.4.0 (\(revision), macos-arm64, unmanaged)"
    )
    ...
    #expect(object["version"] as? String == "0.4.0")
  ...
  private var packagedBuildInformation: String {
    "{\"schema_version\":1,\"version\":\"0.4.0\",\"revision\":\"\(revision)\"}\n"
  }
