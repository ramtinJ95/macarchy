...
extension AdapterContractTests {
  ...
  enum ExhaustedSketchyBarSettle: Sendable {
    case observedDrift
    case queryTimeout
  }

  @Test(arguments: [ExhaustedSketchyBarSettle.observedDrift, .queryTimeout])
  func sketchyBarStopsPollingWhenTheSettleWindowIsExhausted(
    _ failure: ExhaustedSketchyBarSettle
  ) async throws {
    try await withTemporaryRoot(named: "macarchy-adapter-tests") { root in
      ...
      let adapter = SketchyBarAdapter(
        ...
        waitForSettle: { settleCount.withLock { $0 += 1 } },
        waitForPresentation: {
          Issue.record("Presentation wait must not run before an observed, settled state")
        }
      )
